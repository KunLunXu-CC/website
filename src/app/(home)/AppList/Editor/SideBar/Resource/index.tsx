...
const Resource = () => {
  ...
  const { folders, side, works, articles, selectKey } = useSelector(
    (state) => ({
  ...
  const treeData = useMemo(() => {
    ...
    const groupArticles = _.groupBy(
      cloneArticles.reduce((total, ele) => {
        // 根据状态来过滤数据
        (!_.isNumber(selectKey) || ele.status === selectKey) &&
          total.push({
            ...ele,
          });
    ...
    cloneFolders.forEach((v) => {
      ...
      v.children = side.openKeys.includes(v.id)
        ? [
            // eslint-disable-line
  ...
  return (
    <>
      ...
      />
      <div onClick={handleAdd} className={scss.add}>
        +
      </div>
    </>
  );
